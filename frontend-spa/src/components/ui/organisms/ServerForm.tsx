...
export function ServerForm({ 
  ...
  const validateForm = useCallback((): boolean => {
    ...
    if (!formData.websocketUrl.trim()) {
      ...
      newErrors.websocketUrl = 'Enter a valid WebSocket URL (ws://host:port, wss://host:port, or [IPv6]:port)';
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <Card className="w-full max-w-md max-h-[90vh] overflow-hidden">
        <CardHeader className="flex-row items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="p-4 space-y-4 overflow-y-auto">
            ...
            <div className="pt-2">
              ...
              {connectionTestResult && (
                <div
                  className={`mt-2 p-2 rounded-md flex items-start gap-2 ${
          ...
          <CardFooter className="p-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
            <Button
              type="button"
              variant="secondary"
              fullWidth
