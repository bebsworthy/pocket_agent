...
export const projectStatesAtom = atom<Map<string, {
  state: ProjectState;
...
export const websocketServicesAtom = atom(
  (get) => {
...
export const websocketConnectionStatesAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const websocketReconnectionAttemptsAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const websocketLastActivityAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const websocketConfigAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const joinedProjectsAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const websocketErrorsAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const serverStatsAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const connectedServersAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
export const totalConnectionsAtom = atom(
  (get) => get(connectedServersAtom).length
);

// Optimized derived atom to check if any server is connecting
export const isAnyServerConnectingAtom = atom(
  (get) => {
    const serverStates = get(websocketServerStatesAtom);
...
const getOrCreateServerState = (serverStates: Map<string, WebSocketServerState>, serverId: string): WebSocketServerState => {
...
export const updateConnectionStatusAtom = atom(
  ...
  (get, set, serverId: string, status: ConnectionStatus) => {
    ...
    newServerStates.set(serverId, {
      ...
      lastActivity: (status === 'connected' || status === 'disconnected') 
...
export const clearMessageQueueAtom = atom(
  null,
...
export const joinProjectAtom = atom(
  null,
  (get, set, serverId: string, projectId: string) => {
    const serverStates = get(websocketServerStatesAtom);
    const newServerStates = new Map(serverStates);
    const currentState = getOrCreateServerState(serverStates, serverId);
...
export const leaveProjectAtom = atom(
  null,
  (get, set, serverId: string, projectId: string) => {
    const serverStates = get(websocketServerStatesAtom);
    const newServerStates = new Map(serverStates);
...
export const clearWebSocketErrorAtom = atom(
  null,
  (get, set, serverId: string) => {
    const serverStates = get(websocketServerStatesAtom);
    const newServerStates = new Map(serverStates);
...
export const updateServerStatsAtom = atom(
  null,
  (get, set, serverId: string, stats: Omit<{
    connections: number;
    projects: number;
    uptime: number;
    version: string;
...
export const setWebSocketConfigAtom = atom(
  null,
  (get, set, serverId: string, config: {
    url: string;
    autoReconnect: boolean;
    maxReconnectAttempts: number;
    reconnectDelay: number;
