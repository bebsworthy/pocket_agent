...
export const serverCountAtom = atom(
  (get) => get(serversAtom).length
);

// Derived atom to check if servers exist
export const hasServersAtom = atom(
  (get) => get(serversAtom).length > 0
);
...
export const connectedServersCountAtom = atom(
  (get) => {
    const connectionStates = get(serverConnectionStatesAtom);
...
export const removeServerAtom = atom(
  null,
...
export const updateServerAtom = atom(
  null,
  (get, set, updatedServer: Server) => {
    const servers = get(serversAtom);
...
export const getServerByIdAtom = atom(
  null,
  (get, _set, serverId: string) => {
    const servers = get(serversAtom);
