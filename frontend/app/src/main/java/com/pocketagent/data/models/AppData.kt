...
class AppDataBuilder {
    ...
    fun addMessages(projectId: String, messageList: List<Message>) = apply { 
        this.messages[projectId] = messageList 
    }
    
    fun lastModified(timestamp: Long) = apply { 
        this.lastModified = timestamp 
    }
    
    fun metadata(metadata: AppMetadata) = apply { 
 ...
 */
fun AppData.getSshIdentitiesSorted(): List<SshIdentity> = 
    sshIdentities.sortedBy { it.name }

/**
 * Get all server profiles sorted by name.
 */
fun AppData.getServerProfilesSorted(): List<ServerProfile> = 
    serverProfiles.sortedBy { it.name }

/**
 * Get all projects sorted by last activity.
 */
fun AppData.getProjectsSorted(): List<Project> = 
    projects.sortedByDescending { it.lastActiveAt ?: it.createdAt }

/**
 * Get projects for a specific server profile.
 */
fun AppData.getProjectsForServer(serverProfileId: String): List<Project> = 
 ...
 */
fun AppData.getProjectMessages(projectId: String): List<Message> = 
    messages[projectId] ?: emptyList()

/**
 * Get total count of all entities.
 */
fun AppData.getTotalEntityCount(): Int = 
    sshIdentities.size + serverProfiles.size + projects.size

/**
 * Check if the data contains any entities.
 */
fun AppData.isEmpty(): Boolean = 
    sshIdentities.isEmpty() && serverProfiles.isEmpty() && projects.isEmpty()

/**
 * Get a summary of data statistics.
 */
fun AppData.getDataSummary(): DataSummary = DataSummary(
