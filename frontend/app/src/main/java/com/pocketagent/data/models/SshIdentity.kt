 ...
 */
fun SshIdentity.markAsUsed(): SshIdentity = copy(lastUsedAt = System.currentTimeMillis())

/**
 * Update the description.
 */
fun SshIdentity.withDescription(description: String?): SshIdentity = copy(description = description)
 ...
 */
fun SshIdentity.toExportModel(): SshIdentityExport =
    SshIdentityExport(
        id = id,
        name = name,
        publicKeyFingerprint = publicKeyFingerprint,
...
object SshIdentityFactory {
     ...
     */
    fun createSample(name: String = "Sample SSH Key"): SshIdentity =
        SshIdentityBuilder()
            .name(name)
     ...
     */
    fun createSamples(count: Int): List<SshIdentity> =
        (1..count).map { index ->
