...
sealed class ValidationResult {
     ...
     */
    fun getFieldErrors(field: String): List<ValidationError> =
        when (this) {
            is Success -> emptyList()
     ...
     */
    inline fun <T> map(transform: () -> T): ValidationResult =
        when (this) {
            is Success ->
                try {
                    transform()
...
data class ValidationError(
    ...
    companion object {
        fun fieldError(
            ...
            message: String,
            code: String? = null,
        ): ValidationError = ValidationError(message, field, Type.FIELD_VALIDATION, code)

        fun businessRuleError(
            message: String,
            field: String? = null,
            code: String? = null,
        ): ValidationError = ValidationError(message, field, Type.BUSINESS_RULE, code)

        fun relationshipError(
            message: String,
            field: String? = null,
            code: String? = null,
        ): ValidationError = ValidationError(message, field, Type.RELATIONSHIP_CONSTRAINT, code)

        fun databaseError(
            message: String,
            field: String? = null,
            code: String? = null,
        ): ValidationError = ValidationError(message, field, Type.DATABASE_CONSTRAINT, code)

        fun customError(
            message: String,
            field: String? = null,
            code: String? = null,
        ): ValidationError = ValidationError(message, field, Type.CUSTOM_RULE, code)
    }
}
...
object ValidationResultUtils {
    ...
    fun combine(vararg results: ValidationResult): ValidationResult =
        results.fold(ValidationResult.Success as ValidationResult) { acc, result ->
            acc.and(result)
        }
    ...
    fun failure(
        message: String,
        field: String? = null,
    ): ValidationResult = ValidationResult.Failure(ValidationError(message, field))

    /**
     * Create a failed validation result with multiple errors.
     */
    fun failure(errors: List<ValidationError>): ValidationResult = ValidationResult.Failure(errors)
