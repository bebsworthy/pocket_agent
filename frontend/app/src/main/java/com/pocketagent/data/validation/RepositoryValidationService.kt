    ...
    constructor(
        ...
        suspend fun validateSshIdentityForUpdate(
            ...
            withContext(Dispatchers.Default) {
                ...
                if (original.publicKeyFingerprint != updated.publicKeyFingerprint) {
                    ...
                    builder.addResult(
                        sshIdentityValidator.validateFingerprintUniqueness(updated.publicKeyFingerprint, existingFingerprints, updated.id),
                    )
                }
        ...
        suspend fun validateServerProfileForCreation(
            ...
            withContext(Dispatchers.Default) {
                ...
                builder.addResult(
                    serverProfileValidator.validateHostnamePortUniqueness(
                        profile.hostname,
                        profile.port,
                        existingHostnamePorts,
                    ),
                )
            ...
            withContext(Dispatchers.Default) {
                ...
                if (original.id != updated.id) {
                    val existingMessageIds =
                        existingData.messages[projectId]
         ...
         */
        suspend fun validateCompleteAppData(data: AppData): ValidationResult =
            withContext(Dispatchers.Default) {
         ...
         */
        suspend fun validateAppDataForSave(data: AppData): ValidationResult =
            withContext(Dispatchers.Default) {
                val builder = ValidationResultBuilder()
        ...
        suspend fun validateWithAsyncConstraints(
            ...
            val allValidations = listOf(validation) + asyncConstraints
            val results = allValidations.map { rule ->
                try {
                    rule()
                } catch (e: Exception) {
         ...
         */
        suspend fun validateRepositoryUrl(url: String): ValidationResult =
            withContext(Dispatchers.Default) {
                val builder = ValidationResultBuilder()
