...
class StorageValidator @Inject constructor(
    ...
    data class ValidationReport(
        ...
        val hasErrors: Boolean get() = results.any { it.severity == ValidationSeverity.ERROR || it.severity == ValidationSeverity.CRITICAL }
    ...
    suspend fun validateStorageFile(filename: String): Result<ValidationResult> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val encryptedDataResult = kotlin.runCatching {
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (duplicates.isNotEmpty()) {
                    val duplicateDetails = duplicates.map { (checksum, files) ->
                ...
                val orphanedFiles = files.filter { filename ->
    ...
    private fun calculateJsonDepth(element: JsonElement): Int {
        return when {
            ...
            element.toString().startsWith("[") -> {
                ...
                if (array == "[]") 1 else 1 + (json.parseToJsonElement(array).let { arr ->
