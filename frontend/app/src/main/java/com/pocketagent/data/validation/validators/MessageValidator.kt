...
class MessageValidator
    ...
    constructor() {
        companion object {
            ...
            private val SUSPICIOUS_PATTERNS =
                listOf(
                    "<script",
                    "javascript:",
                    "data:text/html",
        ...
        fun validateBusinessRules(message: Message): ValidationResult {
            ...
            builder.addResult(validateMessageTypeContent(message))
            builder.addResult(validatePartialMessageConstraints(message))
            builder.addResult(validateTimestampReasonableness(message))

            return builder.build()
        }

        /**
         * Validates content appropriateness for message type.
         */
        private fun validateMessageTypeContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()

            when (message.type) {
                MessageType.USER_INPUT -> {
                    builder.addResult(validateUserInputContent(message))
                }
                MessageType.CLAUDE_RESPONSE -> {
                    builder.addResult(validateClaudeResponseContent(message))
                }
                MessageType.SYSTEM_MESSAGE -> {
                    builder.addResult(validateSystemMessageContent(message))
                }
                MessageType.ERROR_MESSAGE -> {
                    builder.addResult(validateErrorMessageContent(message))
                }
                MessageType.STATUS_UPDATE -> {
                    builder.addResult(validateStatusUpdateContent(message))
                }
            }
        ...
        /**
         * Validates user input content.
         */
        private fun validateUserInputContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
        ...
        /**
         * Validates Claude response content.
         */
        private fun validateClaudeResponseContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
        ...
        /**
         * Validates system message content.
         */
        private fun validateSystemMessageContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
         ...
         */
        private fun validateErrorMessageContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
        ...
        /**
         * Validates status update content.
         */
        private fun validateStatusUpdateContent(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
        ...
        /**
         * Validates partial message constraints.
         */
        private fun validatePartialMessageConstraints(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
         ...
         */
        private fun validateTimestampReasonableness(message: Message): ValidationResult {
            val builder = ValidationResultBuilder()
        ...
        fun validateForUpdate(
            ...
            validateImmutableFields(original, updated, builder)
            validateContentChanges(original, updated, builder)
            validatePartialFlagChanges(original, updated, builder)

            return builder.build()
        }

        /**
         * Validates that immutable fields haven't changed.
         */
        private fun validateImmutableFields(
            original: Message,
            updated: Message,
        ...
        /**
         * Validates content changes based on message type.
         */
        private fun validateContentChanges(
            original: Message,
            updated: Message,
            builder: ValidationResultBuilder,
        ) {
            if (original.content != updated.content) {
                when (original.type) {
                    MessageType.CLAUDE_RESPONSE -> {
                        validateClaudeResponseContentUpdate(original, updated, builder)
                    }
                    MessageType.USER_INPUT -> {
                        builder.addBusinessRuleError(
        ...
        /**
         * Validates Claude response content updates (streaming completion).
         */
        private fun validateClaudeResponseContentUpdate(
            original: Message,
            updated: Message,
            builder: ValidationResultBuilder,
        ) {
        ...
        /**
         * Validates isPartial flag changes.
         */
        private fun validatePartialFlagChanges(
            original: Message,
            updated: Message,
            builder: ValidationResultBuilder,
        ) {
         ...
         */
        fun validateIdUniqueness(
            id: String,
            existingIds: List<String>,
            excludeId: String? = null,
        ): ValidationResult {
            val conflictingIds =
                existingIds.filter { existingId ->
