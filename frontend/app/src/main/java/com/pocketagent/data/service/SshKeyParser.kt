 ...
 */
@Singleton
class SshKeyParser @Inject constructor() {
    ...
    companion object {
        ...
        private val OPENSSH_PUBLIC_KEY_PREFIXES = listOf(
    ...
    fun parsePrivateKey(
        ...
        return try {
            val detectedFormat = if (format == SshKeyFormat.AUTO_DETECT) {
     ...
     */
    fun generateKeyPair(keyType: SshKeyType, keySize: Int): KeyPair {
     ...
     */
    fun formatPrivateKey(privateKey: PrivateKey, format: SshKeyFormat): String {
     ...
     */
    fun formatPublicKey(publicKey: PublicKey, format: SshKeyFormat): String {
    ...
    private fun parseOpenSshPrivateKey(keyData: String, passphrase: String?): KeyPair? {
        ...
        try {
            ...
            val base64Content = trimmed.substring(startIndex + startMarker.length, endIndex)
                .replace("\n", "")
                .replace("\r", "")
                .replace(" ", "")
    ...
    private fun parsePemPrivateKey(keyData: String, passphrase: String?): KeyPair? {
        ...
        try {
            ...
            val publicKey = when (privateKey.algorithm) {
                "RSA" -> {
    ...
    private fun parsePkcs8PrivateKey(keyData: String, passphrase: String?): KeyPair? {
    ...
    private fun parsePuttyPrivateKey(keyData: String, passphrase: String?): KeyPair? {
