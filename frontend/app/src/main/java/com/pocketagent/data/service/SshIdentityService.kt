...
class SshIdentityService
    ...
    constructor(
        ...
        suspend fun createSshIdentity(
            ...
            return try {
                val result = withContext(Dispatchers.Default) {
                    validateAndCreateSshIdentity(name, privateKeyData, keyFormat, passphrase, description)
                }
                result
            } catch (e: Exception) {
        ...
        /**
         * Validates all requirements and creates the SSH identity.
         */
        private suspend fun validateAndCreateSshIdentity(
            name: String,
            privateKeyData: String,
            ...
            val keyPair = sshKeyParser.parsePrivateKey(privateKeyData, keyFormat, passphrase)
                ?: return serviceFailure("Failed to parse private key")
            ...
            if (existingIdentities.any { it.publicKeyFingerprint == fingerprint }) {
                return serviceFailure("SSH key with this fingerprint already exists")
            }
            ...
            val validationError = performSshIdentityValidations(name, identity, existingIdentities)
            ...
            return ServiceResult.success(identity)
        }
        
        /**
         * Performs comprehensive validation for SSH identity creation.
         */
        private suspend fun performSshIdentityValidations(
            name: String,
            identity: SshIdentity,
            existingIdentities: List<SshIdentity>
        ): ServiceResult<SshIdentity>? {
        ...
        suspend fun updateSshIdentity(
            ...
            return try {
                val result = validateAndUpdateSshIdentity(id, name, description)
                result
            } catch (e: Exception) {
        ...
        /**
         * Validates all requirements and updates the SSH identity.
         */
        private suspend fun validateAndUpdateSshIdentity(
            id: String,
            name: String?,
            description: String?
        ): ServiceResult<SshIdentity> {
            ...
            // Perform all validations
            val validationError = performUpdateValidations(existing, updated, id, name)
            if (validationError != null) return validationError
        ...
        /**
         * Performs validation for SSH identity update.
         */
        private suspend fun performUpdateValidations(
        ...
        suspend fun deleteSshIdentity(id: String): ServiceResult<Unit> {
            ...
            return try {
                val result = validateAndDeleteSshIdentity(id)
                result
            } catch (e: Exception) {
        ...
        /**
         * Validates dependencies and deletes the SSH identity.
         */
        private suspend fun validateAndDeleteSshIdentity(id: String): ServiceResult<Unit> {
            val identity = repository.getSshIdentityById(id)
                ?: return serviceFailure("SSH identity not found")
            ...
            if (serverProfiles.isNotEmpty()) {
                val projectNames = serverProfiles.take(3).joinToString(", ") { it.name }
                val ellipsis = if (serverProfiles.size > 3) "..." else ""
                val message = "Cannot delete SSH identity. It is used by ${serverProfiles.size} server profile(s): $projectNames$ellipsis"
         ...
         */
        suspend fun filterSshIdentities(criteria: SshIdentityFilterCriteria): ServiceResult<List<SshIdentity>> {
        ...
        suspend fun exportSshKey(
            ...
            return try {
                val result = performSshKeyExport(id, format, includePrivateKey)
                result
            } catch (e: Exception) {
        ...
        /**
         * Performs the SSH key export operation.
         */
        private suspend fun performSshKeyExport(
            id: String,
            format: SshKeyFormat,
            ...
            return if (includePrivateKey) {
                exportPrivateKey(identity, format)
            } else {
                exportPublicKey(identity, format)
            }
        }
         ...
         */
        private fun exportPrivateKey(identity: SshIdentity, format: SshKeyFormat): ServiceResult<String> {
            ...
            return ServiceResult.success(exported)
        }
        
        /**
         * Exports the public key in the specified format.
         */
        private fun exportPublicKey(identity: SshIdentity, format: SshKeyFormat): ServiceResult<String> {
        ...
        suspend fun validateSshKey(
            ...
            return try {
                withContext(Dispatchers.Default) {
                    val keyPair =
                        sshKeyParser.parsePrivateKey(keyData, format, passphrase)
        ...
        suspend fun markAsUsed(id: String): ServiceResult<Unit> {
            ...
            return try {
                val identity =
                    repository.getSshIdentityById(id)
                        ?: return serviceFailure("SSH identity not found")
