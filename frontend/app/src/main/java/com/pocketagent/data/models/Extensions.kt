 ...
 */
fun AppData.getActiveProjects(): List<Project> = projects.filter { it.status == ProjectStatus.ACTIVE }

/**
 * Get all connected servers.
 */
fun AppData.getConnectedServers(): List<ServerProfile> = serverProfiles.filter { it.status == ConnectionStatus.CONNECTED }

/**
 * Get recently used SSH identities (used in last 30 days).
 */
fun AppData.getRecentlyUsedIdentities(): List<SshIdentity> = sshIdentities.filter { it.isRecentlyUsed() }

/**
 * Get projects with errors.
 */
fun AppData.getProjectsWithErrors(): List<Project> = projects.filter { it.status == ProjectStatus.ERROR }

/**
 * Get servers with errors.
 */
fun AppData.getServersWithErrors(): List<ServerProfile> = serverProfiles.filter { it.status == ConnectionStatus.ERROR }

/**
 * Get total message count across all projects.
 */
fun AppData.getTotalMessageCount(): Int = messages.values.sumOf { it.size }

/**
 * Get projects by status.
 */
fun AppData.getProjectsByStatus(status: ProjectStatus): List<Project> = projects.filter { it.status == status }

/**
 * Get servers by status.
 */
fun AppData.getServersByStatus(status: ConnectionStatus): List<ServerProfile> = serverProfiles.filter { it.status == status }
...
fun List<SshIdentity>.sortByUsage(): List<SshIdentity> =
    sortedWith(
        compareByDescending<SshIdentity> { it.lastUsedAt ?: 0 }
            .thenBy { it.name },
    )
...
fun List<ServerProfile>.sortByStatus(): List<ServerProfile> =
    sortedWith(
        compareBy<ServerProfile> { it.status.ordinal }
...
fun List<Project>.sortByActivity(): List<Project> =
    sortedWith(
        compareByDescending<Project> { it.lastActiveAt ?: 0 }
            .thenBy { it.name },
    )
 ...
 */
fun List<Message>.sortByTimestamp(): List<Message> = sortedBy { it.timestamp }

/**
 * Get only recently used SSH identities.
 */
fun List<SshIdentity>.recentlyUsed(): List<SshIdentity> = filter { it.isRecentlyUsed() }

/**
 * Get only connected server profiles.
 */
fun List<ServerProfile>.connected(): List<ServerProfile> = filter { it.status == ConnectionStatus.CONNECTED }

/**
 * Get only active projects.
 */
fun List<Project>.active(): List<Project> = filter { it.status == ProjectStatus.ACTIVE }

/**
 * Get only error messages.
 */
fun List<Message>.errors(): List<Message> = filter { it.type == MessageType.ERROR_MESSAGE }

/**
 * Get only user messages.
 */
fun List<Message>.userMessages(): List<Message> = filter { it.type == MessageType.USER_INPUT }

/**
 * Get only Claude messages.
 */
fun List<Message>.claudeMessages(): List<Message> = filter { it.type == MessageType.CLAUDE_RESPONSE }
 ...
 */
fun AppData.toExportFormat(): AppDataExport =
    AppDataExport(
        version = version,
 ...
 */
fun AppData.createSummary(): DataStateSummary =
    DataStateSummary(
 ...
 */
fun AppData.isCompletelyEmpty(): Boolean = sshIdentities.isEmpty() && serverProfiles.isEmpty() && projects.isEmpty() && messages.isEmpty()
 ...
 */
fun AppData.updateTimestamp(): AppData = copy(lastModified = System.currentTimeMillis())

/**
 * Get entity count by type.
 */
fun AppData.getEntityCounts(): Map<String, Int> =
    mapOf(
 ...
 */
fun AppData.getHealthStatus(): DataHealthStatus =
    when {
