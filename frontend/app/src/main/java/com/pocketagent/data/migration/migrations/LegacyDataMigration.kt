...
class LegacyDataMigration @Inject constructor(
    ...
    private suspend fun migratePreferencesData(
        ...
        if (!legacySshKeyName.isNullOrBlank() && !legacySshKeyData.isNullOrBlank()) {
            val legacySshIdentity = SshIdentity(
     ...
     */
    private fun mergeLegacyData(currentData: AppData, legacyData: AppData): AppData {
 ...
 */
@Singleton
class LegacyDataCleanupMigration @Inject constructor(
    @ApplicationContext private val context: Context
) : BaseDataMigration() {
    ...
    override suspend fun migrate(
        ...
        return withContext(Dispatchers.IO) {
            ...
            val tempFiles = context.filesDir.listFiles { _, name ->
