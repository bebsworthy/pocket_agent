 ...
 */
@Singleton
class NetworkConfigurationValidator @Inject constructor() {
    ...
    companion object {
        ...
        private val IP_V6_PATTERN = Pattern.compile(
    ...
    suspend fun validateConfiguration(
        ...
        if (!hostnameValidation.isValid) {
            errors.add((hostnameValidation as ValidationResult.Failure).error)
        }
        
        // Validate ports
        val sshPortValidation = validatePort(sshPort, "SSH")
        if (!sshPortValidation.isValid) {
            errors.add((sshPortValidation as ValidationResult.Failure).error)
        }
        
        val wrapperPortValidation = validatePort(wrapperPort, "wrapper")
        if (!wrapperPortValidation.isValid) {
            errors.add((wrapperPortValidation as ValidationResult.Failure).error)
        }
        ...
        if (!portAssignmentValidation.isValid) {
            errors.add((portAssignmentValidation as ValidationResult.Failure).error)
        }
        
        return if (errors.isEmpty()) {
            ValidationResult.Success
        } else {
            ValidationResult.Failure(errors.first()) // Return first error for simplicity
        }
    }
     ...
     */
    fun validatePort(port: Int, portType: String): ValidationResult {
     ...
     */
    private fun validatePortAssignments(sshPort: Int, wrapperPort: Int): ValidationResult {
     ...
     */
    suspend fun isPortReachable(hostname: String, port: Int): Boolean {
        return try {
            withContext(Dispatchers.IO) {
     ...
     */
    fun getRecommendations(hostname: String, sshPort: Int, wrapperPort: Int): List<String> {
     ...
     */
    fun detectCommonIssues(hostname: String, sshPort: Int, wrapperPort: Int): List<String> {
