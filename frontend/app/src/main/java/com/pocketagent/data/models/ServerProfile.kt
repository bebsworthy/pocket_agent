 ...
 */
fun ServerProfile.markAsDisconnected(): ServerProfile = copy(status = ConnectionStatus.DISCONNECTED)

/**
 * Mark as connecting.
 */
fun ServerProfile.markAsConnecting(): ServerProfile = copy(status = ConnectionStatus.CONNECTING)

/**
 * Mark as error state.
 */
fun ServerProfile.markAsError(): ServerProfile = copy(status = ConnectionStatus.ERROR)
 ...
 */
fun ServerProfile.toExportModel(): ServerProfileExport =
    ServerProfileExport(
        id = id,
        name = name,
        hostname = hostname,
