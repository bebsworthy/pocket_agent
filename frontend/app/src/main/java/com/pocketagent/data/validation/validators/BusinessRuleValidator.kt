 ...
 */
@Singleton
class BusinessRuleValidator @Inject constructor() {
    ...
    fun validateProjectBusinessRules(
        ...
        projects.forEach { project ->
            // Active projects should have recent activity
            ...
            // Error projects should have error messages
            ...
            // Check for path/name consistency
            val pathBaseName = project.projectPath.split("/", "\\").lastOrNull()?.lowercase()
            val projectNameNormalized = project.name.lowercase().replace("\\s+".toRegex(), "")
            
            if (pathBaseName != null && pathBaseName != projectNameNormalized && 
    ...
    fun validateDataConsistency(data: AppData): ValidationResult {
        ...
        val allEntityIds = (data.sshIdentities.map { it.id } + 
                           data.serverProfiles.map { it.id } + 
                           data.projects.map { it.id }).toSet()
     ...
     */
    fun validateEntityDeletion(entityType: String, entityId: String, data: AppData): ValidationResult {
        val builder = ValidationResultBuilder()
