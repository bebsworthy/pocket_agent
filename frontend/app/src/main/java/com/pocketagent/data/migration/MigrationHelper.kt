...
class MigrationHelper @Inject constructor(
    ...
    suspend fun initializeRepositoryWithMigration(
        ...
        return try {
            ...
            val migrationResult = repository.initializeWithMigration(
                migrationManager,
                migrationConfiguration
            )
            
            // Create summary
            val summary = if (migrationResult != null) {
                MigrationSummary(
     ...
     */
    suspend fun performDataHealthCheck(
        repository: SecureDataRepository
    ): HealthCheckResult {
    ...
    suspend fun getMigrationStatus(
        ...
        return try {
            ...
            MigrationStatusInfo(
                ...
                recentMigrations = history.takeLast(5).map { entry ->
