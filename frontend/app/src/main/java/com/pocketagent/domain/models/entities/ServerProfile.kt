...
import com.pocketagent.common.constants.NetworkConstants
...
data class ServerProfile(
    ...
    val name: String,
    val hostname: String,
    val port: Int = NetworkConstants.DEFAULT_SSH_PORT,
    val username: String,
    val sshIdentityId: String,
    val wrapperPort: Int = NetworkConstants.DEFAULT_WRAPPER_PORT,
     ...
     */
    fun markAsConnected(): ServerProfile =
        copy(
    ...
    companion object {
        ...
        const val MAX_HOSTNAME_LENGTH = SizeConstants.MAX_HOSTNAME_LENGTH
        const val MAX_USERNAME_LENGTH = SizeConstants.MAX_USERNAME_LENGTH_SSH
        const val MIN_PORT = NetworkConstants.MIN_PORT_NUMBER
        const val MAX_PORT = NetworkConstants.MAX_PORT_NUMBER
        ...
        private fun validateWrapperPort(wrapperPort: Int) {
            if (wrapperPort !in MIN_PORT..MAX_PORT) {
                val message = "Wrapper port must be between $MIN_PORT and $MAX_PORT"
                throw ValidationException("wrapperPort", wrapperPort, message)
            }
        }
...
data class ServerConnectionSettings(
    ...
    val keepAliveInterval: Long = DEFAULT_KEEP_ALIVE_INTERVAL,
    val maxRetries: Int = DEFAULT_MAX_RETRIES,
    val useCompression: Boolean = true,
    val strictHostKeyChecking: Boolean = true,
) {
    companion object {
        const val DEFAULT_CONNECTION_TIMEOUT = NetworkConstants.SERVER_DEFAULT_CONNECTION_TIMEOUT // 30 seconds
        const val DEFAULT_KEEP_ALIVE_INTERVAL = NetworkConstants.SERVER_DEFAULT_KEEP_ALIVE_INTERVAL // 1 minute
        const val DEFAULT_MAX_RETRIES = NetworkConstants.SERVER_DEFAULT_MAX_RETRIES
    }
}
...
class ServerProfileBuilder {
    ...
    private var port: Int = NetworkConstants.DEFAULT_SSH_PORT
    private var username: String = ""
    private var sshIdentityId: String = ""
    private var wrapperPort: Int = NetworkConstants.DEFAULT_WRAPPER_PORT
 ...
 */
fun ServerProfile.toBuilder(): ServerProfileBuilder =
 ...
 */
fun serverProfile(block: ServerProfileBuilder.() -> Unit): ServerProfile = ServerProfileBuilder().apply(block).build()
