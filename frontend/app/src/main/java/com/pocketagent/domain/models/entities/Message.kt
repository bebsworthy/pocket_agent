...
data class Message(
     ...
     */
    fun addAttachment(attachment: MessageAttachment): Message =
        copy(
     ...
     */
    fun removeAttachment(attachmentId: String): Message =
        copy(
     ...
     */
    fun removeMetadata(key: String): Message =
        copy(
     ...
     */
    fun getPreview(): String = if (content.length <= PREVIEW_LENGTH) content else "${content.take(PREVIEW_LENGTH)}..."
    ...
    companion object {
        ...
        private fun validateContent(content: String) {
            if (content.length > MAX_CONTENT_LENGTH) {
                val message = "Message content too long (max $MAX_CONTENT_LENGTH chars)"
                throw ValidationException("content", content, message)
            }
        }
    }
}
...
data class MessageAttachment(
    ...
    companion object {
        ...
        private fun validateContent(content: String) {
            if (content.length > MAX_CONTENT_LENGTH) {
                val message = "Attachment content too long (max $MAX_CONTENT_LENGTH chars)"
                throw ValidationException("content", content, message)
            }
        }
    }
 ...
 */
fun Message.toBuilder(): MessageBuilder =
    MessageBuilder()
        .id(id)
        .content(content)
        .type(type)
 ...
 */
fun MessageAttachment.toBuilder(): MessageAttachmentBuilder =
 ...
 */
fun message(block: MessageBuilder.() -> Unit): Message = MessageBuilder().apply(block).build()

/**
 * Creates a new message attachment builder.
 */
fun messageAttachment(block: MessageAttachmentBuilder.() -> Unit): MessageAttachment = MessageAttachmentBuilder().apply(block).build()
