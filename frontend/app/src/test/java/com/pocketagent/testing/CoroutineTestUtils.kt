...
object FlowTestUtils {
     ...
     */
    suspend fun <T> kotlinx.coroutines.flow.Flow<T>.collectInTest(
        testScope: TestScope
    ): List<T> {
     ...
     */
    suspend fun <T> kotlinx.coroutines.flow.Flow<T>.firstInTest(
        testScope: TestScope
    ): T {
        var result: T? = null
        val job = testScope.launch {
    ...
    suspend fun <T> kotlinx.coroutines.flow.StateFlow<T>.waitForValue(
        ...
        while (this.value != expectedValue) {
            if (System.currentTimeMillis() - startTime > timeoutMillis) {
                throw AssertionError("StateFlow did not emit expected value $expectedValue within ${timeoutMillis}ms. Current value: ${this.value}")
            }
...
object SharedFlowTestUtils {
    ...
    suspend fun <T> kotlinx.coroutines.flow.SharedFlow<T>.collectCount(
        ...
        while (emissions.size < count) {
            if (System.currentTimeMillis() - startTime > timeoutMillis) {
                job.cancel()
                throw AssertionError("SharedFlow did not emit $count values within ${timeoutMillis}ms. Got ${emissions.size} values: $emissions")
            }
            testScope.advanceTimeBy(50)
        }
