...
class ServerConnectionTesterTest {
    ...
    fun `testConnection should return success for valid configuration`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(
            mockk {
        ...
        val result = connectionTester.testConnection(
            hostname = "httpbin.org", // Public test service
            port = 80,
            username = "testuser",
            sshIdentityId = "ssh-1",
            wrapperPort = 8080,
    ...
    @Test
    fun `testConnection should handle DNS resolution failure`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(
            mockk {
                every { id } returns "ssh-1"
    ...
    @Test
    fun `testConnection should handle port unreachable`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(
            mockk {
                every { id } returns "ssh-1"
    ...
    @Test
    fun `testConnection should handle SSH identity not found`() = runTest {
        // Arrange
    ...
    @Test
    fun `testMultipleConnections should handle empty profile list`() = runTest {
        // Act
    ...
    @Test
    fun `testMultipleConnections should handle single profile`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(
            mockk {
                every { id } returns "ssh-1"
    ...
    @Test
    fun `testMultipleConnections should respect concurrency limit`() = runTest {
        // Arrange
