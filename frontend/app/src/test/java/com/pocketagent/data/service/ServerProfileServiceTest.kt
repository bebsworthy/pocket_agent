...
class ServerProfileServiceTest {
    ...
    @Test
    fun `createServerProfile should create profile successfully`() = runTest {
        // Arrange
    ...
    @Test
    fun `createServerProfile should fail when SSH identity not found`() = runTest {
        // Arrange
    ...
    @Test
    fun `createServerProfile should fail when validation fails`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(testSshIdentity)
    ...
    @Test
    fun `createServerProfile should fail when name already exists`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(testSshIdentity)
        every { mockValidator.validateForCreation(any()) } returns ValidationResult.Success
        every { mockValidator.validateNameUniqueness(any(), any()) } returns ValidationResult.Failure(
    ...
    @Test
    fun `createServerProfile with connection test should update status on success`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(testSshIdentity)
        every { mockValidator.validateForCreation(any()) } returns ValidationResult.Success
        every { mockValidator.validateNameUniqueness(any(), any()) } returns ValidationResult.Success
    ...
    @Test
    fun `getServerProfile should return profile when found`() = runTest {
        // Arrange
    ...
    @Test
    fun `getServerProfile should return failure when not found`() = runTest {
        // Arrange
    ...
    @Test
    fun `updateServerProfile should update profile successfully`() = runTest {
        // Arrange
        coEvery { mockRepository.getServerProfileById("server-1") } returns testServerProfile
    ...
    @Test
    fun `updateServerProfile should validate SSH identity when changed`() = runTest {
        // Arrange
    ...
    @Test
    fun `deleteServerProfile should delete successfully when no dependencies`() = runTest {
        // Arrange
        coEvery { mockRepository.getServerProfileById("server-1") } returns testServerProfile
    ...
    @Test
    fun `deleteServerProfile should fail when profile has dependencies`() = runTest {
        // Arrange
        val mockProject = mockk<com.pocketagent.data.models.Project> {
    ...
    @Test
    fun `listServerProfiles should return sorted profiles`() = runTest {
        // Arrange
    ...
    @Test
    fun `listServerProfiles should filter by SSH identity`() = runTest {
        // Arrange
    ...
    @Test
    fun `searchServerProfiles should find matching profiles`() = runTest {
        // Arrange
    ...
    @Test
    fun `searchServerProfiles should return empty for blank query`() = runTest {
        // Act
    ...
    @Test
    fun `filterServerProfiles should apply all criteria`() = runTest {
        // Arrange
        val now = System.currentTimeMillis()
        val profile1 = testServerProfile.copy(
            id = "1", 
            sshIdentityId = "ssh-1", 
            status = ConnectionStatus.CONNECTED,
            createdAt = now - 1000
        )
        val profile2 = testServerProfile.copy(
            id = "2", 
            sshIdentityId = "ssh-2", 
            status = ConnectionStatus.DISCONNECTED,
            createdAt = now - 2000
        )
        val profile3 = testServerProfile.copy(
            id = "3", 
            sshIdentityId = "ssh-1", 
            status = ConnectionStatus.CONNECTED,
            createdAt = now - 3000
        ...
        // Act
        val criteria = ServerProfileFilterCriteria(
            sshIdentityId = "ssh-1",
    ...
    @Test
    fun `testConnection should return success result`() = runTest {
        // Arrange
    ...
    @Test
    fun `testMultipleConnections should test all profiles`() = runTest {
        // Arrange
        val expectedResults = mapOf(
            "server-1" to ConnectionTestResult(
                connectionStatus = ConnectionTestResult.Status.SUCCESS,
                message = "Connection successful",
                responseTimeMs = 500
            ),
            "server-2" to ConnectionTestResult(
    ...
    @Test
    fun `updateConnectionStatus should update profile status`() = runTest {
        // Arrange
        coEvery { mockRepository.getServerProfileById("server-1") } returns testServerProfile
        coEvery { mockRepository.updateServerProfile(any()) } just Runs
    ...
    @Test
    fun `getUsageStatistics should return correct stats`() = runTest {
        // Arrange
        val mockProject1 = mockk<com.pocketagent.data.models.Project> {
        ...
        val mockProject2 = mockk<com.pocketagent.data.models.Project> {
            every { serverProfileId } returns "server-1"
            every { status } returns mockk {
    ...
    @Test
    fun `validateNetworkConfiguration should return validation report`() = runTest {
        // Arrange
    ...
    @Test
    fun `service should handle repository exceptions gracefully`() = runTest {
        // Arrange
        coEvery { mockRepository.getServerProfileById("server-1") } throws DataException.CorruptedDataException("Data corrupted", RuntimeException())
    ...
    @Test
    fun `service should handle validation exceptions gracefully`() = runTest {
        // Arrange
        coEvery { mockSshIdentityService.getSshIdentity("ssh-1") } returns ServiceResult.success(testSshIdentity)
